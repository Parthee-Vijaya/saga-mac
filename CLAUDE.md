...
`MultilingualASRRouter` (Bridge/) vælger mellem `CanaryASRBridge` (dansk + 25 EU-sprog), `HviskeASRBridge` (dansk premium via WhisperKit) og `AppleSpeechBridge` (tamilsk + andre) baseret på `effectiveLanguage` plus `preferredDanishEngine`. Routing-prioritet: dansk + Hviske valgt + Hviske ready → Hviske; ellers Canary hvis supporteret; ellers Apple Speech. Ved Hviske-fejl falder routeren tilbage til Canary; fejler begge kastes `ASRRouterError.bothDanishEnginesFailed`. Resultatet stemples med `engineLabel` for den engine der faktisk kørte — HUD/stats læser den frem for at gætte fra sprogvalget. Bridges conformer til `DanishASRBridging`/`AppleSpeechBridging`-protocols så routeren kan testes med mocks (se `MultilingualASRRouterTests`). `HviskeASRBridge` loader mlpackages fra søsterprojektet `~/projekter/aktive/hviske-coreml/models/whisperkit/syvai_hviske-v3/` og compiler dem til `.mlmodelc` ved første brug (cached i `~/Library/Application Support/Saga/HviskeCompiled/`). Der er INGEN automatisk download af Hviske-modeller — de skal være på disk. Picker i Settings → Voice → "Dansk ASR-engine".
